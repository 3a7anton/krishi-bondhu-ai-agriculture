  ...
  async getPesticideRecommendations(params: {
    ...
    try {
      const prompt = `
        ...
        Provide specific product names available in Bangladesh/India market.
      `;
      ...
      const response = await axios.post(
        this.openRouterAPI,
        {
          ...
          messages: [
            {
              role: 'system',
              content: `You are an agricultural AI assistant specializing in ${task}. Provide accurate, practical advice for farmers in Bangladesh/India region.`
            },
            {
              role: 'user',
              content: prompt
