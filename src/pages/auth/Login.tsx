  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        setTimeout(() => {
          // Route users to appropriate dashboard based on their role in database
          const email = formData.email.toLowerCase();
          
          // Default routing based on email pattern (fallback)
          if (email.includes('farmer') || email.startsWith('farmer')) {
            navigate('/farmer/dashboard');
          } else if (email.includes('customer') || email.startsWith('customer')) {
            navigate('/customer/dashboard');
          } else if (email.includes('delivery') || email.startsWith('delivery')) {
            navigate('/delivery/dashboard');
          } else if (email.includes('warehouse') || email.startsWith('warehouse')) {
            navigate('/warehouse/dashboard');
          } else if (email.includes('admin') || email.startsWith('admin')) {
            navigate('/admin/dashboard');
          } else {
            // Default to general dashboard for unrecognized patterns
            navigate('/dashboard');
          }
        }, 1000);
      }
